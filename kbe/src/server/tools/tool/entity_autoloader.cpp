...
void EntityAutoLoader::onEntityAutoLoadCBFromDBMgr(Network::Channel* pChannel, MemoryStream& s)
{
	...
	for(int i=0; i<size; ++i)
	{
		...
		if(PyObject_HasAttrString(Tool::getSingleton().getEntryScript().get(), "onAutoLoadEntityCreate") > 0)
		{
		...
		{
			Tool::getSingleton().createEntityFromDBID(EntityDef::findScriptModule(entityType)->getName(), dbid, NULL, 
