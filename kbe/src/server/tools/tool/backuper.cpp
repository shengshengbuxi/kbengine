...
void Backuper::tick()
{
	int32 periodInTicks = (int32)secondsToTicks(ServerConfig::getSingleton().getTool().backupPeriod, 0);
	if (periodInTicks == 0)
		return;

	// 这里对备份的entity做一下分批操作
	// 大概算法是配置上填写的备份周期换算成tick数量， 每个tick备份一部分entity
	float numToBackUpFloat = float(Tool::getSingleton().pEntities()->size()) / periodInTicks + backupRemainder_;
	...
	{
		Entity * pEntity = Tool::getSingleton().findEntity(backupEntityIDs_.back());
...
void Backuper::createBackupTable()
{
	...
	Entities<Entity>::ENTITYS_MAP::const_iterator iter = Tool::getSingleton().pEntities()->getEntities().begin();

	for(; iter != Tool::getSingleton().pEntities()->getEntities().end(); ++iter)
	{
