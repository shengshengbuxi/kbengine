...
class Entity:
    ...
    def clientIn(self, destID):
        """        
        功能说明：
        通过这个方法可以访问指定实体ID（指定实体必须绑定了客户端）的客户端中本实体的方法，只有本实体在指定实体View范围内才会同步到客户端。它只能在一个real实体上被调用。
    ...
    def navigate(self, destination, velocity, distance, maxMoveDistance, maxSearchDistance, faceMovement, layer, checkFlag, userData):
        """        
        功能说明：
        ...
        checkFlag
        uint64，是否检查标记，0表示不检查，非0表示检查标记匹配值
    ...
    def navigatePathPoints(self, destination, maxSearchDistance, layer, checkFlag):
        """        
        功能说明：
    ...
    def entityInWitnessed(self, entityID):
        """
        功能说明：
        这个函数是判断实体ID是否是当前实体的观察者（玩家）
    ...
    def setDetailLevel(self, detailLevelsOrLevel: int|list, radius=None):
        """
        功能说明：
        设置实体的细节级别，用于优化同步性能。
        ...
        @detailLevelsOrLevel
        int|list，细节级别，可以是一个整数，也可以是一个列表。
            如果是一个列表，索引是细节级别，值是细节级别对应的半径。


        @radius
        float，细节级别的半径，如果为None，则使用实体的默认半径。
        """
        pass
    ...
    def getDetailLevel(self):
        ...
        返回：
        tuple, 包含细节级别和半径的元组，index是细节级别，value是细节级别对应的半径。
...
class Space:
    def setTileFlag(self, x, y, camp):
        """
        设置tiledmap指定区块 进行标记，标记会在调用navigate 和 navigatePathPoints 判断
        坐标x、y 是空间坐标
        引擎会换算成对应的区块坐标
        camp 是标记的阵营，0表示无标记，1表示阵营1，2表示阵营2
        """
        pass
...
@property
def DETAIL_LEVEL_NEAR():
    pass

@property
def DETAIL_LEVEL_MEDIUM():
    pass

@property
def DETAIL_LEVEL_FAR():
    pass