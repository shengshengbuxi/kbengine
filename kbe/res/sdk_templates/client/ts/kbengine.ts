...
export module KBEngine {
     ...
     * @param classType 类型
     */
    export function module<T extends typeof Entity | typeof EntityComponent>(name, t: T): T {
        KBEngine[name] = t;
        return t;
    }
    //#endregion
    ...
    const COMPONENT_TYPE_BASE = 6;
    const COMPONENT_TYPE_CELL = 6;
    export class DATATYPE_ENTITY_COMPONENT {
        ...
        createFromStream(stream: MemoryStream) {
            let ctype = stream.readUint32();

            let ownerID = stream.readInt32();
            let utype = stream.readUint16();
            let propertyCount = stream.readUint16();
            ...
            let entity = KBEngineapp.entities[ownerID];
            if (entity == undefined) {
            }
            let entityModuleDef = KBEngineModuledefs[entity.className];
            let entityComponents = entityModuleDef["components"];
            let propComUType = 0;
            let propComAliasID = -1;
            const usePropertyDescrAlias = entityModuleDef["usePropertyDescrAlias"];
            let componentModuleDef = KBEngineModuledefs[this.utype];

            for (let k in entityComponents) {
                const entityPropertyDesc = entityComponents[k];
                if (entityPropertyDesc[4].utype == this.utype) {

                    propComUType = entityPropertyDesc[0];
                    propComAliasID = entityPropertyDesc[1];
            ...
            const comName = entityComponents[usePropertyDescrAlias ? propComAliasID : propComUType][2];
            let comObj = entity[comName];

            if (comObj == undefined) {
                const clsName = componentModuleDef["name"];
                let comclass = KBEngineapp.getentityclass(clsName);
                if (comclass == undefined) {
                    // ERROR_MSG("没有实现组件：" + clsName);

                    return;
                }

                comObj = new comclass();
                comObj.className = clsName;

                entity[comName] = comObj;
            }

            comObj.owner = entity;

            // if (ctype == COMPONENT_TYPE_CELL) {
            // if (entity.inWorld || ctype == COMPONENT_TYPE_CELL) {
            if (!comObj.cell) {
                comObj.cell = new EntityCall();
                comObj.cell.id = ownerID;
                comObj.cell.className = comObj.className;
                comObj.cell.type = ENTITYCALL_TYPE_CELL;
            }
            // }
            // } else {
            if (!comObj.base) {

                comObj.base = new EntityCall();
                comObj.base.id = ownerID;
                comObj.base.className = comObj.className;
                comObj.base.type = ENTITYCALL_TYPE_BASE;
            }
            // }

            comObj.entityComponentPropertyID = propComUType;


            if (propertyCount > 0) {
                for (let i = 0; i < propertyCount; ++i) {
                    let _t_child_utype = 0, _t_utype = 0;
                    if (componentModuleDef.usePropertyDescrAlias) {
                    ...
                    var componentPData = componentModuleDef.propertys[_t_child_utype];
    ...
    KBEngineDatatypes["ENTITY_COMPONENT"] = new DATATYPE_ENTITY_COMPONENT();
    KBEngineDatatypes["BLOB"] = new DATATYPE_BLOB();
    KBEngineDatatypes["TEXT"] = new DATATYPE_TEXT();
    //#endregion
    ...
    export class INT64 {
        ...
        static fromString(v: string) {
            ...
            const lo = Number(bigInt & BigInt(0xffffffff));
        ...
        toString() {
            ...
            const highBigInt = BigInt(this.hi);
            const lowBigInt = BigInt(this.lo);

            const combinedBigInt = (highBigInt << BigInt(32)) | lowBigInt;


            return result + combinedBigInt.toString();
        }
    }
    ...
    export class UINT64 {
        ...
        compare(v: UINT64 | string): boolean {
            if (v.constructor == UINT64) {
                if (this.hi == v.hi && this.lo == v.lo)
                    return true;
            } else {
                if (this.toString() == v) {
                    return true;
        ...
        gt(v: UINT64 | number) {
            ...
            if (this.hi > hi || this.hi == hi && this.lo > lo) {
                return true;
            }
    ...
    class EventInfo {
        callback: Function;
        object: any;
        constructor(callbackfn: Function, object: Object = null) {
    ...
    export class EventManager {
         ...
         * @param evtName 事件名称
         * @param callback 回调方法
         * @param object 回调方法this对象
         */
        register(evtName: string, callback: Function, object?: Object) {
            //  strCallback: string) {
            ...
            var info = new EventInfo(callback, object);
            evtlst.push(info);
        }
         ...
         * @param evtName 事件名称
         * @param callback 回调
         * @param object 回调this对象
         */
        deregister(evtName: string, callback: Function, object?: Object) {
            var evtlst = this._events[evtName];
            if (evtlst == undefined) {
                return;
            }
            while (true) {
                ...
                for (var i = 0; i < evtlst.length; i++) {
                    var info = evtlst[i];
                    object && object == info.object || true;

                    if (info.callback == callback && (!object || object == info.object)) {
            ...
            this.removeFiredEvent(evtName, callback, object);
        }
        ...
        removeFiredEvent(evtName: string, callback: Object, object?: Object) {
            var firedEvents = this._firedEvents;
            while (true) {
                var found = false;
                for (var i = 0; i < firedEvents.length; i++) {
                    var evt = firedEvents[i];
                    if ((evtName == "" || evt.evtName == evtName) && evt.evtInfo.callback == callback && (!object || object == evt.evtInfo.object)) {
        ...
        fire(evtName: string, ...args: any[]) {
            ...
            for (var i = 0; i < evtlst.length; i++) {
                var info: EventInfo = evtlst[i];

                if (!this._isPause) {
                    try {
                        info.callback.apply(info.object, args);
                    } catch (error) {
                        ERROR_MSG(error);
                    }
                } else {
        ...
        resume() {
            ...
            while (firedEvents.length > 0) {
                var evt: FiredEvent = firedEvents.shift();
                var info: EventInfo = evt.evtInfo;
                var args = evt.args;
    ...
    export class MemoryStream {
        ...
        append(stream: MemoryStream, offset: number, size: number) {
            ...
            if (size > this.space()) {
                ...
                let newBuff = new Uint8Array(this.buffer.byteLength + size * 2);


                newBuff.set(new Uint8Array(this.buffer));

                this.buffer = newBuff.buffer;
            }

            let buff = new Uint8Array(this.buffer, this.wpos, size);
            buff.set(new Uint8Array(stream.buffer, offset, size), 0);
        ...
        readInt64(): INT64 {
            return new INT64(this.readInt32(), this.readInt32());
        }
        ...
        readStream(): MemoryStream {
            ...
            return new MemoryStream(buf.buffer);
        }
        ...
        static _objects: Array<MemoryStream> = [];


        static createObject(): MemoryStream {
    ...
    export class Bundle {
        ...
        stream: MemoryStream = MemoryStream.createObject();
        numMessage = 0;
        messageLengthBuffer: Uint8Array | null;
        messageLength = 0;
        msgtype = null;
        static _objects: Array<Bundle> = [];
    ...
    function bindReader(argType) {
        ...
        else if (argType == datatype2id["TEXT"]) {
            return reader.readStream;
        }
        else {
            return reader.readStream;
    ...
    class Message {
        ...
        handleMessage(msgstream) {
            ...
            if (this.args.length <= 0) {
                ...
                const args = this.createFromStream(msgstream);
                try {
                    this.handler.apply(KBEngineapp, args);
                } catch (error) {
                    ERROR_MSG(error);
                }
            }
    ...
    export class Vector3 {
        ...
        /**向量归一化*/
        normalize() {
            let count = this.x + this.y + this.z;
            this.x /= count;
            this.y /= count;
            this.z /= count;
            return this;
    ...
    export class Entity {
        ...
        callPropertysSetMethods() {
            ...
            for (var name in currModule.propertys) {
                ...
                let datatype = propertydata[4];
                var setmethod = propertydata[5];
                var flags = propertydata[6];
                var oldval = this[name];

                if (datatype.constructor == DATATYPE_ENTITY_COMPONENT) {
                    this[name]?.callPropertysSetMethods();
                } else {
                    if (setmethod != null) {
                        // base类属性或者进入世界后cell类属性会触发set_*方法
        ...
        getComponent<T extends typeof EntityComponent>(t: T): InstanceType<T> {
            if (!KBEngineModuledefs[t.name]) {
                return null;
            }

            const entityModuleDef = KBEngineModuledefs[this.className];
            const entityComponents = entityModuleDef["components"];
            const componentModuleDef = KBEngineModuledefs[t.name];

            for (let k in entityComponents) {
                const entityPropertyDesc = entityComponents[k];
                const comDataType: DATATYPE_ENTITY_COMPONENT = entityPropertyDesc[4];

                if (KBEngineModuledefs[comDataType.utype] == componentModuleDef) {
                    return this[entityPropertyDesc[2]];
                }
            }
        ...
        baseCall(funName: string, ...args: any[]) {
            ...
            if (!KBEngineApp.app.socket) {
                KBEngineApp.app.errorCallback?.call(null);
            }
            ...
            WARNING_MSG(`[KBE] --> ${this.className} ${funName}\n`, JSON.stringify(args), args, "\n");
        ...
        enterWorld() {
            ...
            for (let k in entityComponents) {
                const entityPropertyDesc: EntityComponent = entityComponents[k];
                try {
                    this[entityPropertyDesc[2]]?.onEnterWorld();
                } catch (error) {
                    ERROR_MSG(error);
                }
            }

            Event.fire(EventTypes.onEnterWorld, this);
        }
        ...
        leaveWorld() {
            ...
            for (let k in entityComponents) {
                const entityPropertyDesc = entityComponents[k];
                this[entityPropertyDesc[2]]?.onLeaveWorld();
            }

            Event.fire(EventTypes.onLeaveWorld, this);
        }
        ...
        enterSpace() {
            ...
            this.onEnterSpace();
            Event.fire(EventTypes.onEnterSpace, this);

            // 要立即刷新表现层对象的位置
            Event.fire(EventTypes.set_position, this);
            Event.fire(EventTypes.set_direction, this);
        }
    ...
    export class EntityComponent {
            ...
            try {
                for (var i = 0; i < args.length; i++) {
                    if (methodArgs[i].isSameType(args[i])) {
                        ...
                        throw new Error("KBEngine.EntityComponent::baseCall: arg[" + i + "] is error!");
                    }
                }
            } catch (e) {
                ERROR_MSG(e.toString());
                ERROR_MSG('KBEngine.EntityComponent::baseCall: args is error!');
                this.base.bundle = null;
                return;
            }
        ...
        cellCall(funName: string, ...args: any[]) {
            if (funName.length < 1) {
                ERROR_MSG('KBEngine.EntityComponent::cellCall: not fount interfaceName!');
                return;
            }

            if (this.cell == undefined) {
                ERROR_MSG('KBEngine.EntityComponent::cellCall: cell is None!');
                return;
            }
            ...
            if (method == undefined) {
                ERROR_MSG("KBEngine.EntityComponent::cellCall: The server did not find the def_method(" + this.className + "." + funName + ")!");
                return;
            }
            ...
            if (methodArgs.length != args.length) {
                ERROR_MSG("KBEngine.EntityComponent::cellCall: args(" + methodArgs.length + "!= " + args.length + ") size is error!");
                return;
            }
            ...
            try {
                for (var i = 0; i < args.length; i++) {
                    if (methodArgs[i].isSameType(args[i])) {
                        ...
                        throw new Error("KBEngine.EntityComponent::cellCall: arg[" + i + "] is error!");
                    }
                }
            } catch (e) {
                ERROR_MSG(e.toString());
                ERROR_MSG('KBEngine.EntityComponent::cellCall: args is error!');
                this.cell.bundle = null;
                return;
            }
    ...
    export class KBEngineArgs {
        ...
        /**心跳时间间隔 */
        serverHeartbeatTick: number = 120;
        /**客户端类型 */
        clientType: number = 5;
    ...
    export class KBEngineApp {
        ...
        private constructor(kbengineArgs: KBEngineArgs) {
            this.args = kbengineArgs;
            KBEngineapp = this;
            // this.ip = this.args.ip;
            // this.port = this.args.port;
            // this.domain = this.args.address;
        ...
        installEvents() {
            Event.register(EventTypes.createAccount, this.createAccount, this);
            Event.register(EventTypes.login, this.login, this);
            Event.register(EventTypes.logout, this.logout, this);
            Event.register(EventTypes.reloginBaseapp, this.reloginBaseapp, this);
            Event.register(EventTypes.bindAccountEmail, this.bindAccountEmail, this);
            Event.register(EventTypes.newPassword, this.newPassword, this);
            Event.register(EventTypes.resetPassword, this.resetPassword, this);
        }

        uninstallEvents() {
            Event.deregister(EventTypes.createAccount, this.createAccount);
            Event.deregister(EventTypes.login, this.login);
            Event.deregister(EventTypes.logout, this.logout);
            Event.deregister(EventTypes.reloginBaseapp, this.reloginBaseapp);
            Event.deregister(EventTypes.bindAccountEmail, this.bindAccountEmail);
            Event.deregister(EventTypes.newPassword, this.newPassword);
            Event.deregister(EventTypes.resetPassword, this.resetPassword);
        }
            ...
            while (stream.length() > 0 || app.fragmentStream != null) {
                if (app.fragmentDatasFlag == app.FragmentDataTypes.FRAGMENT_DATA_UNKNOW) {
                    ...
                    if (app.fragmentStream != null && app.fragmentStream.length() >= app.currMsgLen) {
                        ...
                        var rpos = stream.rpos + app.currMsgLen;
                        stream.wpos = rpos;
        ...
        writeFragmentMessage(FragmentDataType, stream, datasize) {
            ...
            if (opsize > 0) {
                KBEngineapp.fragmentStream = MemoryStream.createObject();
        ...
        send(msg) {
            if (!KBEngineapp.socket) {
                KBEngineapp.errorCallback?.call(null);
            }
            else {
        ...
        */
        ...
        createDataTypeFromStream(stream: MemoryStream, canprint) {
            ...
            */
            ...
            if (name == "FIXED_DICT") {
                ...
                let datatype = new DATATYPE_ENTITY_COMPONENT();
                KBEngineDatatypes[valname] = datatype;
        ...
        Client_onImportClientEntityDef(stream) {
            ...
            while (stream.length() > 0) {
                ...
                let self_components = currModuleDefs["components"];
                ...
                if (Class == undefined) {
                    ...
                    let tmpClassName = scriptmodule_name;
                    KBEngine[scriptmodule_name] = class extends Entity {
                        constructor() {
                            super();
                    ...
                    Class = KBEngine[scriptmodule_name];
                }
                ...
                while (propertysize > 0) {
                    ...
                    if (utype.constructor == DATATYPE_ENTITY_COMPONENT) {
                        if (aliasID != -1) {
                            self_components[aliasID] = savedata;
                        } else {
                            self_components[properUtype] = savedata;
                        }
                    }
        ...
        Client_onVersionNotMatch(stream) {
            ...
            Event.fire(EventTypes.onVersionNotMatch, KBEngineapp.clientVersion, KBEngineapp.serverVersion);
        }
        ...
        Client_onScriptVersionNotMatch(stream) {
            ...
            Event.fire(EventTypes.onScriptVersionNotMatch, KBEngineapp.clientScriptVersion, KBEngineapp.serverScriptVersion);
        }
        ...
        getServerAddr(port?: number, ip?: string) {
            const wsOrWssRegex = /^(ws|wss):\/\//;
            ...
            if (address.startsWith("wss://")) {
                if (port) {
                    if (portRegex.test(address)) {
                        address = address.replace(portRegex, `:${port}`);
                    } else {
                        address += `:${port}`;
                    }
                }
                ...
                if (ip) {
                    if (ipRegex.test(address)) {
                        address = address.replace(ipRegex, ip);
                    }
                }
        ...
        createAccount_loginapp(noconnect) {
            if (noconnect) {
                var serverAddr = this.getServerAddr();
        ...
        login_loginapp(noconnect) {
            if (noconnect) {
                ...
                console.error(`kbengine--login_loginapp--serverAddr:${JSON.stringify(serverAddr)}}`);
        ...
        login_baseapp(noconnect) {
            if (noconnect) {
                Event.fire(EventTypes.onLoginBaseapp);
                var serverAddr = this.getServerAddr(KBEngineapp.baseappTcpPort, KBEngineapp.baseappIP);
        ...
        Client_onLoginFailed(args) {
            ...
            Event.fire(EventTypes.onLoginFailed, failedcode, KBEngineapp.serverdatas);
        }
        ...
        Client_onCreatedProxies(rndUUID, eid, entityType) {
            ...
            if (entity == undefined) {
                ...
                const entityModuleDef = KBEngineModuledefs[entity.className];
                const entityComponents = entityModuleDef["components"];

                for (let k in entityComponents) {
                    const entityPropertyDesc = entityComponents[k];
                    try {
                        entity[entityPropertyDesc[2]]?.__init__();
                    } catch (error) {
                        ERROR_MSG(error);
                    }
                }
                    ...
                    if (_t_child_utype != 0) {
                        ...
                        if (comPropertyData !== undefined) {
                            ...
                            if (comObj) {
                                ...
                                // ERROR_MSG("没有实现组件：" + comPropertyData[2]);
                            }
                        }
                    }
                } else {
            ...
            if (propertyUtype == 0) {
                ...
                if (methoddata !== undefined) {
                    if (entity[methoddata[2]] != undefined) {
                        ...
                        WARNING_MSG(`[KBE] <-- property ${entity.className} ${methoddata[2]}\n`, JSON.stringify(args), args, "\n");
                        try {
                            var isError = this.ErrorCode_MSG(methoddata[2], args[0]);
                            if (!isError) {
                ...
                if (comObj) {
                    if (KBEngineModuledefs[comObj.className].useMethodDescrAlias) {
                        methodUtype = stream.readUint8();
                    } else {
                        methodUtype = stream.readUint16();
                    }
                    ...
                    if (true || comObj[methoddata[2]] != null) {
                        var args = [];
                        var argsdata = methoddata[3];
                        for (var i = 0; i < argsdata.length; i++) {
                            args.push(argsdata[i].createFromStream(stream));
                        }
                        WARNING_MSG(`[KBE] <-- ${comObj.className} ${methoddata[2]}\n`, JSON.stringify(args), args, "\n");
                        try {
                            var isError = this.ErrorCode_MSG(methoddata[2], args[0]);
                            if (!isError) {
                                comObj[methoddata[2]] != null && comObj[methoddata[2]].apply(comObj, args);
                            }
                        } catch (error) {
                            ERROR_MSG(error);
                        }
        ...
        ErrorCode_MSG(methodName: string, errCode): boolean {
            return false;//等服务端的返回和策划的表处理好之后打开
            let isResp = methodName.includes("resp");
            if (!isResp) {
                return false;
            }
            let errCodeNum = Number(errCode);
            if (isNaN(errCodeNum)) {
                return false;
            }
            Event.fire(EventTypes.onErrorCode, errCodeNum);
            return true;
        }
        ...
        Client_onEntityEnterWorld(stream) {
            ...
            if (entity == undefined) {
                ...
                for (let k in entityComponents) {
                    const entityPropertyDesc = entityComponents[k];
                    const entityComponent: EntityComponent = entity[entityPropertyDesc[2]];
                    if (entityComponent) {
                        entityComponent.cell = new EntityCall();

                        entityComponent.cell.id = eid;
                        entityComponent.cell.className = entityComponent.className;
                        entityComponent.cell.type = ENTITYCALL_TYPE_CELL;
                    } else {
                        // ERROR_MSG("没有实现组件：" + entityPropertyDesc[2]);
                    }
                }
            ...
            if (KBEngineapp.entity_id > 0 && eid != KBEngineapp.entity_id) {
                ...
                for (var i = 0; i < KBEngineapp.controlledEntities.length; i++) {
                    if (KBEngineapp.controlledEntities[i] != eid) {
                        ...
                        Event.fire(EventTypes.onLoseControlledEntity);
                    }
                }
        ...
        Client_onCreateAccountResult(stream) {
            ...
            Event.fire(EventTypes.onCreateAccountResult, retcode, datas);
        ...
        clearEntities(isAll) {
            ...
            if (!isAll) {
                ...
                for (var eid in KBEngineapp.entities) {
                    ...
                    for (let k in entityComponents) {
                        const entityPropertyDesc = entityComponents[k];
                        entity[entityPropertyDesc[2]]?.onDestroy();
                    }
                ...
                for (var eid in KBEngineapp.entities) {
                    ...
                    const entityModuleDef = KBEngineModuledefs[KBEngineapp.entities[eid].className];
                    const entityComponents = entityModuleDef["components"];

                    for (let k in entityComponents) {
                        const entityPropertyDesc = entityComponents[k];
                        KBEngineapp.entities[eid][entityPropertyDesc[2]]?.onDestroy();
                    }
        ...
        Client_setSpaceData(spaceID, key, value) {
            ...
            Event.fire(EventTypes.onSetSpaceData, spaceID, key, value);
        }
        ...
        Client_delSpaceData(spaceID, key) {
            ...
            Event.fire(EventTypes.onDelSpaceData, spaceID, key);
        }
        ...
        Client_onUpdateBasePos(x, y, z) {
            ...
            if (entity != undefined && entity.isControlled) {
                ...
                Event.fire(EventTypes.updatePosition, entity);
        ...
        _updateVolatileData(entityID, x, y, z, yaw, pitch, roll, isOnGround, isOptimized) {
            ...
            if (changeDirection == true) {
                Event.fire(EventTypes.set_direction, entity);
                done = true;
            }
        ...
        Client_onReqAccountResetPasswordCB(failedcode) {
            Event.fire(EventTypes.onResetPassword, failedcode);
            if (failedcode != 0) {
        ...
        Client_onReqAccountBindEmailCB(failedcode) {
            Event.fire(EventTypes.onBindAccountEmail, failedcode);
            if (failedcode != 0) {
        ...
        Client_onReqAccountNewPasswordCB(failedcode) {
            Event.fire(EventTypes.onNewPassword, failedcode);
            if (failedcode != 0) {
